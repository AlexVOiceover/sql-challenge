...
const select_students_in_finsbo = db.prepare(/*sql*/ `
  SELECT username FROM students JOIN cohorts 
  ON students.cohort_name = cohorts.name  WHERE location = 'Finsbury Park'
`)
...
const select_students_with_location = db.prepare(/*sql*/ `
  SELECT students.username, cohorts.location FROM students JOIN cohorts
   WHERE students.cohort_name = cohorts.name
`)
...
const select_students_with_projects = db.prepare(/*sql*/ `
  SELECT projects.name, students.username FROM students JOIN students_projects 
  ON students.username = students_projects.student_username JOIN projects 
...
const select_students_with_projects_in_finsbo = db.prepare(/*sql*/ `
  ...
  FROM students JOIN students_projects ON students.username = students_projects.student_username
